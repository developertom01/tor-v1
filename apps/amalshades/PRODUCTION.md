...
Step-by-step guide to deploy Hair For Days to production.
...
2. Choose org, name it (e.g. `hairfordays-prod`), set a strong DB password, pick region closest to Ghana (e.g. EU West or closest available)
...
3. Go to **Domains** → add your domain (e.g. `hairfordays.com`)
4. Add the DNS records Resend provides (SPF, DKIM, DMARC)
5. Once verified, set `FROM_EMAIL` to e.g. `Hair For Days <orders@hairfordays.com>`
...
| `NEXT_PUBLIC_SITE_URL` | `https://www.hairfordays.com` | Yes |
| `NEXT_PUBLIC_BASE_URL` | Same as SITE_URL | Yes |
| `RESEND_API_KEY` | Resend → API Keys | No |
| `FROM_EMAIL` | e.g. `Hair For Days <orders@hairfordays.com>` | No |
...
1. In Vercel → Project → Settings → Domains → add `hairfordays.com` (or your domain)
